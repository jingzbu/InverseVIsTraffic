...
with open('../temp_files/link_length_dict_journal_insert_links.json', 'r') as json_file:
...
with open('../temp_files/path-link_incidence_journal_insert_links.txt', 'r') as the_file:
    # path counts
    i = 0  
    for row in the_file:
        if '->' in row:
...
with open('../temp_files/path-link_incidence_journal_insert_links.txt', 'r') as the_file:
    ...
    for row in the_file:
        if '->' in row:
            ...
            i = i + 1
    assert(i == r)
zdump(A, '../temp_files/path-link_incidence_matrix_journal.pkz')
