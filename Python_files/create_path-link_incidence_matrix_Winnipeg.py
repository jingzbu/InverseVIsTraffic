...
link_label_dict = zload('../temp_files/link_label_dict_Winnipeg_network.pkz')
link_label_dict_ = zload('../temp_files/link_label_dict_Winnipeg_network_.pkz')

with open('../temp_files/link_length_dict_Winnipeg.json', 'r') as json_file:
...
with open('../temp_files/numRoutes_Winnipeg.json', 'r') as json_file:
...
with open('../temp_files/path-link_incidence_Winnipeg.txt', 'r') as the_file:
    # path counts
    i = 0  
    for row in the_file:
        if '->' in row:
            ...
            i = i + 1
    assert(i == r)
zdump(A, '../temp_files/path-link_incidence_matrix_Winnipeg.pkz')
...
zdump(length_of_route_list, '../temp_files/length_of_route_list_Winnipeg.pkz')

# length_of_route_list[139]

with open('../temp_files/od_pair_label_dict_Winnipeg.json', 'r') as json_file:
    OD_pair_label_dict = json.load(json_file)
...
zdump(OD_pair_route_dict, '../temp_files/OD_pair_route_dict_Winnipeg.pkz')

with open('../temp_files/OD_pair_route_dict_Winnipeg.json', 'w') as json_file:
...
zdump(P, '../temp_files/OD_pair_route_incidence_Winnipeg.pkz')
# zdump(P, '../temp_files/logit_route_choice_probability_matrix_Winnipeg.pkz')
