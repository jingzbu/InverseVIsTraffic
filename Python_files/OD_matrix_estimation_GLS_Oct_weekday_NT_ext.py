...
week_day_Oct_list = [1, 2, 3, 4, 5, 8, 9, 10, 11, 12, 15, 16, 22, 18, 19, 22, 23, 24, 25, 26, 29, 30, 31]
...
for link_idx in range(74):
    for day in week_day_Oct_list: 
        for minute_idx in range(120):
            ...
            link_day_minute_Oct_list.append(link_day_minute_Oct_dict_ext_JSON[key] ['NT_flow_minute'][minute_idx])

# print(len(link_day_minute_Oct_list))

x = np.matrix(link_day_minute_Oct_list)
x = np.matrix.reshape(x, 74, 2760)
...
with open('../temp_files/OD_demand_matrix_Oct_weekday_NT_ext.txt', 'w') as the_file:
    idx = 0
