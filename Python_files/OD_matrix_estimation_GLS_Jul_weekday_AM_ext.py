...
for link_idx in range(64):
    for day in week_day_Jul_list: 
        for minute_idx in range(120):
            ...
            link_day_minute_Jul_list.append(link_day_minute_Jul_dict_ext_JSON[key] ['AM_flow_minute'][minute_idx])

# print(len(link_day_minute_Jul_list))

x = np.matrix(link_day_minute_Jul_list)
x = np.matrix.reshape(x, 64, 720)
...
with open('../temp_files/OD_demand_matrix_Jul_weekday_AM_ext.txt', 'w') as the_file:
    idx = 0
