...
with open('../temp_files/link_day_minute_Jul_dict_journal_JSON.json', 'r') as json_file:
...
for link_idx in range(number_of_links):
    for day in range(32)[1:]: 
        ...
        AM_flow_list.append(link_day_minute_Jul_dict_JSON[key] ['AM_flow'])
        MD_flow_list.append(link_day_minute_Jul_dict_JSON[key] ['MD_flow'])
        PM_flow_list.append(link_day_minute_Jul_dict_JSON[key] ['PM_flow'])
        NT_flow_list.append(link_day_minute_Jul_dict_JSON[key] ['NT_flow'])
        for minute_idx in range(120):
            AM_flow_minute_list.append(link_day_minute_Jul_dict_JSON[key] ['AM_flow_minute'][minute_idx])
            MD_flow_minute_list.append(link_day_minute_Jul_dict_JSON[key] ['MD_flow_minute'][minute_idx])
            PM_flow_minute_list.append(link_day_minute_Jul_dict_JSON[key] ['PM_flow_minute'][minute_idx])
            NT_flow_minute_list.append(link_day_minute_Jul_dict_JSON[key] ['NT_flow_minute'][minute_idx])

x_AM_flow = np.matrix(AM_flow_list)
x_AM_flow = np.matrix.reshape(x_AM_flow, number_of_links, 31)
x_AM_flow = np.nan_to_num(x_AM_flow)

x_MD_flow = np.matrix(MD_flow_list)
x_MD_flow = np.matrix.reshape(x_MD_flow, number_of_links, 31)
x_MD_flow = np.nan_to_num(x_MD_flow)

x_PM_flow = np.matrix(PM_flow_list)
x_PM_flow = np.matrix.reshape(x_PM_flow, number_of_links, 31)
x_PM_flow = np.nan_to_num(x_PM_flow)

x_NT_flow = np.matrix(NT_flow_list)
x_NT_flow = np.matrix.reshape(x_NT_flow, number_of_links, 31)
x_NT_flow = np.nan_to_num(x_NT_flow)

x_AM_flow_minute = np.matrix(AM_flow_minute_list)
x_AM_flow_minute = np.matrix.reshape(x_AM_flow_minute, number_of_links, 3720)
x_AM_flow_minute = np.nan_to_num(x_AM_flow_minute)

x_MD_flow_minute = np.matrix(MD_flow_minute_list)
x_MD_flow_minute = np.matrix.reshape(x_MD_flow_minute, number_of_links, 3720)
x_MD_flow_minute = np.nan_to_num(x_MD_flow_minute)

x_PM_flow_minute = np.matrix(PM_flow_minute_list)
x_PM_flow_minute = np.matrix.reshape(x_PM_flow_minute, number_of_links, 3720)
x_PM_flow_minute = np.nan_to_num(x_PM_flow_minute)

x_NT_flow_minute = np.matrix(NT_flow_minute_list)
x_NT_flow_minute = np.matrix.reshape(x_NT_flow_minute, number_of_links, 3720)
x_NT_flow_minute = np.nan_to_num(x_NT_flow_minute)

print(link_day_minute_Jul_dict_JSON['link_0_1'] ['AM_flow_minute'][0])
...
for j in range(np.size(x_AM_flow, 1)):
    ...
    y_AM_flow.append(flow_conservation_adjustment_journal(y_AM_flow_0))
    y_MD_flow.append(flow_conservation_adjustment_journal(y_MD_flow_0))
    y_PM_flow.append(flow_conservation_adjustment_journal(y_PM_flow_0))
    y_NT_flow.append(flow_conservation_adjustment_journal(y_NT_flow_0))
...
for j in range(np.size(x_AM_flow_minute, 1)):
    ...
    y_AM_flow_minute.append(flow_conservation_adjustment_journal(y_AM_flow_minute_0))
    y_MD_flow_minute.append(flow_conservation_adjustment_journal(y_MD_flow_minute_0))
    y_PM_flow_minute.append(flow_conservation_adjustment_journal(y_PM_flow_minute_0))
    y_NT_flow_minute.append(flow_conservation_adjustment_journal(y_NT_flow_minute_0))
...
link_day_minute_Jul_dict_JSON_adjusted = {}
...
for link_idx in range(number_of_links):
    ...
    for day in range(32)[1:]:   
        ...
        data = {'link_idx': link_idx, 'day': day, \
             'init_node': link_day_minute_Jul_dict_JSON[key]['init_node'], \
             'term_node': link_day_minute_Jul_dict_JSON[key]['term_node'], \
             'AM_capac': link_day_minute_Jul_dict_JSON[key]['AM_capac'], \
             'MD_capac': link_day_minute_Jul_dict_JSON[key]['MD_capac'], \
             'PM_capac': link_day_minute_Jul_dict_JSON[key]['PM_capac'], \
             'NT_capac': link_day_minute_Jul_dict_JSON[key]['NT_capac'], \
             'free_flow_time': link_day_minute_Jul_dict_JSON[key]['free_flow_time'], \
             'length': link_day_minute_Jul_dict_JSON[key]['length'], \
        ...
        link_day_minute_Jul_dict_JSON_adjusted[key] = data

# Writing JSON data
with open('../temp_files/link_day_minute_Jul_dict_journal_JSON_adjusted.json', 'w') as json_file:
    json.dump(link_day_minute_Jul_dict_JSON_adjusted, json_file)
