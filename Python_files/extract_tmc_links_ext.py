...
tmc_list_ext_link_dict = {}
for idx in range(33)[1:]:
    ########## extract tmc info for link_1
    # load attribute table link_1 data
    wb_link = openpyxl.load_workbook(data_folder + 'filtered_INRIX_attribute_table_ext_link_%d.xlsx' %(idx))
    ...
    for i in xrange(2, 1 + sheet_link.max_row):
        tmc_list_ext_link.append(sheet_link.cell(row=i, column=2).value.encode('utf-8')) 
    tmc_list_ext_link_dict[str(idx)] = tmc_list_ext_link


zdump(tmc_list_ext_link_dict, '../temp_files/tmc_list_ext_links_dict.pkz')
