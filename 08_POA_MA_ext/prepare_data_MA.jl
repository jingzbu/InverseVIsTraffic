...
function lengthRouteVec(linkCostDic)
    ...
    the_file = open("../temp_files/path-link_incidence_MA-Falls.txt", "r")
...
function furInfo()
    ...
    numRoutes = readall("../temp_files/numRoutes_MA.json")
    numRoutes = JSON.parse(numRoutes)

    #load OD pair-route incidence
    odPairRoute = readall("../temp_files/od_pair_route_incidence_MA.json")
    odPairRoute = JSON.parse(odPairRoute)

    #load link-route incidence
    linkRoute = readall("../temp_files/link_route_incidence_MA.json")
    linkRoute = JSON.parse(linkRoute)

    link_label_dict = readall("../temp_files/link_label_dict_MA.json")
    link_label_dict = JSON.parse(link_label_dict)

    link_label_dict_ = readall("../temp_files/link_label_dict_MA_.json")
    link_label_dict_ = JSON.parse(link_label_dict_)

    link_length_dict = readall("../temp_files/link_length_dict_MA.json")
    link_length_dict = JSON.parse(link_length_dict)

    OD_pair_route_dict = readall("../temp_files/OD_pair_route_dict_MA.json")
