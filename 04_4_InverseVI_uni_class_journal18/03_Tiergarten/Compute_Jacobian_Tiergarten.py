...
def jacobianSpiess(numNodes, numLinks, numODpairs, od_pairs, link_list_js, link_length_list):
    Tiergarten = nx.DiGraph()

    Tiergarten.add_nodes_from(range(numNodes+1)[1:])

    Tiergarten_weighted_edges = [(int(link_list_js[i].split(',')[0]), int(link_list_js[i].split(',')[1]), \
                             link_length_list[i]) for i in range(len(link_list_js))]

    Tiergarten.add_weighted_edges_from(Tiergarten_weighted_edges)

    path = nx.all_pairs_dijkstra_path(Tiergarten)
